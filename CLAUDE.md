...
Dev-skill 是一个 skill 工厂 — 在这里创建和维护 Claude Code Skills。
...
3. 运行 `scripts/install.ps1`（Windows）或 `scripts/install.sh`（macOS/Linux）将 skill 链接到 `~/.claude/skills/`
...
校验规则参考 `~/.claude/skills/skill-creator/scripts/quick_validate.py`。
关键约束：
