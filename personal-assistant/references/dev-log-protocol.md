...
### R007 [14:30] [P1-user-auth] P2: 实现注册 API
...
### R008 [16:45] [P1-user-auth] P2: 实现登录 API
...
- **Sub-plan and Phase annotation** — 当存在活跃子计划时，轮次标题必须同时标注子计划和内部 Phase：`### RXXX [HH:MM] [P1-user-auth] P2: [简短描述]`。子计划名与 `docs/dep/plans/P<phase>-<name>.md` 文件名一致（不含扩展名）。无子计划时省略子计划标注，有子计划但单 Phase 时省略 Phase 标注。
...
### R007 [14:30] [P1-user-auth] P2: QF: Fixed login timeout in src/auth.py:42 — `abc1234`
```
